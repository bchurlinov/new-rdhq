...
const UpcomingRaces = (): ReactElement => {
    ...
    useEffect(() => {
        if (races.upcoming.page !== currentPage)
            dispatch(getAsyncRaces({ filter: "upcoming", page: currentPage }));
    }, [dispatch, races.upcoming, currentPage]);
    ...
    return (
        <PageWithLoader isLoading={races.upcoming.isLoading || races.upcoming.page === 0} rows={7}>
            <div className="races__wrap">
                <div className={`races__inner-wrap ${races.upcoming.isLoading ? "isLoading" : ""}`}>
                    {renderRacesContent(races.upcoming.isLoading, races.upcoming.results)}
                </div>
                {races.upcoming.count > 10 && (
                    <div className="races__pagination">
                        <Pagination
                            onChange={(page: number) => paginationHandler(page)}
                            current={currentPage}
                            total={races.upcoming.count}
                            showTitle
                        />
                    </div>
                )}
